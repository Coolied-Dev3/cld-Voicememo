...
| 「見積書を送る」「資料をまとめる」（日時の指定なし） | やること | 期限は登録日が初期値。完了ボタンで完了（To Do 側も完了に）。Microsoft 連携済みなら To Do に自動登録 |
| 「明日15時に山田さんと打ち合わせ」「金曜までに週報を提出」「今日の午前中に連絡」（日付や時刻の指定あり） | 予定 | 日時を抽出（時刻なしは終日、午前中・午後・夕方などはおおよその時間枠）。Microsoft 連携済みなら Outlook 予定表に自動登録。未連携なら **Outlook に登録** ボタン／.ics |
| 「牛乳と卵を買う」「買い物、シャンプー」 | 買い物 | チェックで完了 |
| 「インボイス制度について調べて」「RAGとは」 | 調べもの | サーバーが Web 検索し、要約＋参照リンクを表示 |

- 振り分けの基本ルール：文頭の指定 → 調べもの・買い物・アイデアの明確なキーワード → **日付や時刻があれば「予定」、なければ「やること」**。
- 既存メモを現在のルールで再分類するには `node server/reclassify.js`（表示のみ）→ `--apply`（更新。Microsoft 連携済みなら To Do / 予定表も付け替え）。
- 各メモには見出しとは別に **備考（メモ）** を付けられます（登録時は入力欄下の「備考（メモ）を追加」、登録後はカードの「備考」／備考欄タップで編集）。Microsoft 連携済みなら、やることは To Do のメモ欄、予定は Outlook 予定の本文にも反映されます。To Do から取り込んだタスクは To Do のメモ欄が備考になります。
...
**To Do → 本システムの同期**：連携済みユーザーについて、サーバーが `MS_SYNC_INTERVAL` 秒（既定 60）ごとに Microsoft To Do の既定リスト「タスク」を差分取得し、
To Do 側で登録・完了・削除されたタスクを「やること」に反映します（取り込んだものはカードに「To Do」マーク）。初回は未完了のタスクだけを取り込みます。
本システム側で完了・削除すると To Do 側にも反映されるため、**取り込んだタスクをアプリで削除すると To Do のタスクも削除されます**。
手動で今すぐ同期するには一覧の更新ボタン、またはサーバー上で `node server/sync-once.js`。
一覧上部の「当日 / 14日以内 / 全表示」で、やることを期限で絞り込めます（既定は当日＝期限切れを含む今日まで）。
